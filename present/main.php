...
$DB = ['host'=>'localhost','name'=>'k9tin','user'=>'root','pass'=>''];
try{
  $pdo = new PDO(
...
if ($_SERVER['REQUEST_METHOD']==='POST' && $action){
  if ($action==='create'){
    if ($selectedType==='DienThoai'){
      ...
      $bh   = (int)($_POST['BaoHanh'] ?? 0);
      $st = $pdo->prepare("INSERT INTO products(type,ten,gia_vnd,hang_sx,bao_hanh_th) VALUES('DienThoai',:ten,:gia,:hang,:bh)");
      $st->execute(['ten'=>$ten,'gia'=>$gia,'hang'=>$hang,'bh'=>$bh]);
      $notice = "Đã thêm điện thoại.";
    } else { // Sach
      $ten = trim($_POST['TenSach'] ?? '');
      $gia = (int)($_POST['DonGiaSach'] ?? 0);
      $tg  = trim($_POST['TacGia'] ?? '');
      $stg = (int)($_POST['SoTrang'] ?? 0);
      $st = $pdo->prepare("INSERT INTO products(type,ten,gia_vnd,tac_gia,so_trang) VALUES('Sach',:ten,:gia,:tg,:st)");
      $st->execute(['ten'=>$ten,'gia'=>$gia,'tg'=>$tg,'st'=>$stg]);
      $notice = "Đã thêm sách.";
    }
  }
  if ($action==='delete'){
